...
`/album`은 원래 기획 보류(H)였으나, 피그마가 아니라 팀에서 받은 별도 목업을
기준으로 mock 데이터로 먼저 만들었다. `LifeFact.lifeStage`(이야기를 인생
시기별로 묶는 필드)는 이번에 새로 추가된 값이라, 이 값을 누가 정하는지(보호자
선택 / AI 추정)는 아직 PM·AI 확인이 필요하다(`docs/architecture/data-contracts.md`
참고).
