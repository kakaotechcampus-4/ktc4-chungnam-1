...
담당 리더: 이서형. 합성 목 데이터로 화면을 연결한 단계이며 녹음, 카메라, 인증과 서버 연동은 아직 없다.
...
동의와 프로필 입력 → 대화 카드 선택 → 면회 사진과 녹음 화면 → 보호자 소감 및 평가 → 처리 중 → 리포트 → 변경 제안 확인. 현재는 목 화면 이동이며 평가 제출과 변경 제안의 실제 저장은 아직 연결되지 않았다. 화면별 상세 경로는 위 표를 따른다.
