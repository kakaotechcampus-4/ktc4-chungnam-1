...
> **관련 문서**: [VLM 벤치마크 리포트](vlm-benchmark-report.md), [이미지 출처와 라이선스](target-image/SOURCES.md), [공통 데이터 계약](../../../docs/architecture/data-contracts.md), [ADR-001 동의 구조와 원본 자료의 임시 처리 경계](../../../docs/architecture/decisions/ADR-001-consent-and-temporary-processing.md)

---

## 1. 전제 및 범위

- 측정 대상은 **17종의 고해상도 공개 자료** 및 **실기기 대조 촬영 원본 2장**이다. 저장소의 `target-image/` 내 공개 자료 17종은 문서 열람용 리사이즈본이며 메타데이터가 남아 있지 않다(5장 참조, 고해상도 원본은 [SOURCES.md](target-image/SOURCES.md) 참조). 실기기 대조군 원본과 정밀 위치 및 기기 식별값은 개인정보 최소화 원칙에 따라 저장소에 수록하지 않는다.
...
| 원자료 | [`metadata-extraction-results.json`](metadata-extraction-results.json) (재배포 가능한 공개 자료 17종 전수) |
...
스마트폰 카메라 설정에 따른 메타데이터 차이를 확인하기 위해 팀원이 제공한 사진 2장을 로컬에서 측정했다. 공개 자료 17종의 측정 결과는 그대로 유지한다. 실기기 대조군은 표본이 2장뿐이므로 일반적인 단말 특성으로 확대 해석하지 않는다. 원본 사진, 정확한 좌표, 고도, 촬영 시각, 펌웨어와 사진 속 식별값은 저장소에 포함하지 않고 범주형 관찰 결과만 남긴다.

| 항목 | 위치 태그 꺼짐 대조군 | 위치 태그 켜짐 대조군 |
| --- | --- | --- |
| 시험 환경 | 같은 Android 스마트폰 카메라 | 같은 Android 스마트폰 카메라 |
| 촬영일시 필드 | 있음 | 있음 |
| **GPS 블록 유무** | **없음** (태그 0개) | **있음** (태그 10개) |
| 위도와 경도 | 없음 | 있음, 실제 값은 저장소에서 제외 |
| 고도 | 없음 | 있음, 실제 값은 저장소에서 제외 |
| `GPSProcessingMethod` | 없음 | `CELLID` (기지국 측위) |
| GPS 시각 | 없음 | 있음, UTC 기준 |

## 5. 제품 적용 결론

이번 측정은 사진 메타데이터를 서비스의 주 데이터가 아니라 AI 분석의 보조 입력으로 사용해야 하는 근거다.

- 촬영일시, GPS와 기기 정보는 누락되거나 리사이즈 과정에서 사라질 수 있다.
- 스캔, 편집과 재촬영으로 원래 사건과 다른 날짜나 위치가 기록될 수 있다.
- 메타데이터가 없어도 사진 등록과 분석을 계속할 수 있어야 한다.
- 메타데이터만으로 인물, 장소, 시기와 사건을 확정하지 않는다.
- 메타데이터와 AI가 만든 문맥 정보는 모두 확인 전 후보로 취급한다.
- 보호자가 확인한 내용만 프로필, 대화 카드의 근거와 스토리에 반영한다.

AI 태그와 문맥 정보의 사실성은 이 측정만으로 검증되지 않는다. 해당 결과는 별도의 VLM 평가를 거쳐야 하며, 모델 결과를 사용자 확인 없이 확정 정보로 저장하지 않는다. 구체적인 저장 경계는 [공통 데이터 계약의 이미지 분석 후보](../../../docs/architecture/data-contracts.md#이미지-분석-후보-imageanalysiscandidate)를 따른다.
