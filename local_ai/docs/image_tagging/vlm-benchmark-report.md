...
> **전제 및 범위 (본 문서의 성격)**
> - 본 문서는 **후보 모델 평가 리포트**이며 최종 모델을 확정하는 문서가 아님. 이후 팀은 [ADR-006](../../../docs/architecture/decisions/ADR-006-server-side-ai-processing.md)에서 VLM의 서버 측 처리를 확정했다. 이 문서의 `온프레미스` 표기는 실험 당시 후보 분류명이며, 실제 서버 운영 위치와 업체는 아직 정하지 않았다.
