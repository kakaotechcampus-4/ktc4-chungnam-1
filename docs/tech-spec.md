...
| FR-002 | 이미지 분석 | 온프레미스 GPU의 VLM이 태그 또는 description 후보를 만들고 메타데이터는 보조 입력으로만 사용하며, 보호자 확인 전 사실로 저장하지 않아야 함 | 실행 위치 확정, 출력 형식과 모델 결정 대기 |
...
| 이미지 분석 | 동의 범위의 온프레미스 GPU 임시 처리 | 실행 위치 확정, 출력 형식과 모델 결정 대기 |
| 녹음 원본 | 단말 우선, 기능상 필요한 경우 서버 최대 24시간 임시 처리 | 경계 확정, 구현 방식 결정 대기 |
| STT와 화자 처리 | 동의 범위의 온프레미스 GPU 처리 | 실행 위치 확정, 모델과 기준 검증 필요 |
| 카드와 리포트 생성 | 로컬 모델 또는 선택적 중계 서버 | 결정 대기 |
| 품질 개선 데이터 | 별도 선택 동의를 받은 가명정보만 대상 | 확정, 수집과 보관 방식 결정 대기 |

MVP의 STT와 VLM은 온프레미스 GPU 1대에서 처리한다. 장비 소유자, 설치 장소, 사양과 운영 방식은 미정이다. 실제 사용자 원본을 처리하기 전 데이터 흐름, 법률 문서, 동의, 암호화와 삭제 확인을 구현한다.
...
| 온프레미스 GPU 기반 STT와 화자 처리의 모델, 품질 기준과 처리 시간은 무엇인가 | AI, BE | 기술 검증 후 |
| 로컬 DB, 암호화와 복구 방식은 무엇인가 | BE, FE | 첫 프로필 구현 전 |
| 온프레미스 GPU의 장비와 운영 방식, 외부 AI 도입 조건은 무엇인가 | BE, AI, PM | 비용과 데이터 흐름 검증 후 |
