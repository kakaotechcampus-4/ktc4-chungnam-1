...
| AI 리드 | 이문영 | 이미지와 음성 모델, STT, 화자 처리, 생성 모델, 품질 및 실패 기준, GPU 서버 구성·배포·운영 |
| BE 리드 | 김민혁 | 단말 데이터 구조와 로컬 DB, 앱·서버 연동, 처리 파이프라인과 데이터 경계 |
...
| 이미지 분석 | 온프레미스 GPU 서버 임시 처리 | 실행 위치 확정, 원본 업로드 계약 갱신 필요 |
| 녹음 원본 | 단말 우선, 기능상 필요한 경우 서버 최대 24시간 임시 처리 | 경계 확정, 구현 방식 결정 대기 |
| STT와 화자 처리 | 온프레미스 GPU 서버 추론 | 실행 위치 확정, 모델과 성능 검증 필요 |
| 카드와 리포트 생성 | 온프레미스 GPU 서버 추론 | 실행 위치 확정, 모델 결정 대기 |
| 품질 개선 데이터 | 별도 선택 동의를 받은 가명정보만 대상 | 확정, 수집과 보관 방식 결정 대기 |

[ADR-006](architecture/decisions/ADR-006-on-premise-gpu-inference.md)에 따라 초기에는 AI 담당자의 자택 GPU를 추론 서버로 사용한다. GPU 서버 임대와 외부 LLM 서비스 도입은 미정이다. 실제 자료 연동 전 데이터 계약, 동의, 전송 암호화와 삭제 증명을 갖춘다.
...
| 단말 녹음과 GPU 서버 처리의 연동 방식, 최소 단말은 무엇인가 | AI, BE | 기술 검증 후 |
| 로컬 DB, 암호화와 복구 방식은 무엇인가 | BE, FE | 첫 프로필 구현 전 |
| GPU 서버 배포 구성은 무엇인가 | AI | 서버 검증 후 |
| 외부 LLM 서비스가 필요한가 | AI, BE, PM | 데이터 흐름 검증 후 |
