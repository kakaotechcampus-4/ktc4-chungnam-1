# 아키텍처 기준

## 현재 결정과 구현

STT와 VLM은 온프레미스 GPU 1대에서 처리하고, 프로필과 확정 전사문 및 회차 기록은 단말 저장을 기본으로 한다. 장비, 모델, API와 운영 방식은 미정이다. [ADR-006](decisions/ADR-006-server-side-ai-processing.md)에 결정 이유와 남은 조건을 기록한다.

현재 Flutter는 합성 목 데이터를 사용하며 FastAPI는 상태 확인과 공통 오류 및 로그의 기준 골격이다. 아래는 개발할 목표 흐름이며 실제 서버와 저장소 연결이 완료됐다는 뜻은 아니다.

## 목표 사용자 흐름

```mermaid
flowchart TD
    P["프로필과 확인된 생애 정보"] --> G["대화 카드 생성과 확인"]
    P --> I{"사진 분석 사용"}
    I -->|동의 후 암호화 전송| V["온프레미스 GPU: VLM 후보"]
    I -->|사용 안 함 또는 동의 거부| G
    V --> C{"보호자가 후보 확인"}
    C -->|승인| P
    C -->|거절 또는 실패: 직접 입력 가능| G
    G --> A{"녹음 동의와 권한"}
    A -->|충족| R["면회와 녹음"]
    A -->|미충족| STOP["녹음과 원본 업로드 차단"]
    R --> T["온프레미스 GPU: STT와 화자 처리"]
    R --> E["보호자 평가와 수동 기록"]
    T --> Q{"계산할 수 있는 결과"}
    Q -->|정상 또는 사용 가능한 구간| O["관찰값과 이야기 후보"]
    Q -->|실패 또는 신뢰도 부족| E
    E --> REP["리포트와 변경 제안"]
    O --> REP
    REP --> U{"보호자 승인"}
    U -->|승인된 사실과 반응만| D[("단말 저장")]
    U -->|거절: 사실에 미반영| KEEP["이번 회차 기록 유지"]
    D --> B["스토리북"]
    D --> G
    D -->|잘못된 반영 되돌리기| REV["변경 취소"]
```

이미지 메타데이터는 VLM의 보조 참고값이다. 태그, 이미지 설명과 결합 출력 중 무엇을 사용할지는 비교 후 정한다. 보호자 평가가 리포트의 입력이라는 현행 계약을 반영했으며, 음성 처리와 평가의 구체적인 실행 순서 및 API는 아직 설계가 필요하다.

## 데이터 경계

- 사용자 동의를 확인한 원본만 암호화 전송한다. 처리 후 즉시 삭제하고 최대 24시간을 넘기지 않으며 삭제 결과를 확인한다.
- 미확인 후보와 실패한 분석을 확정 사실로 저장하지 않는다. 계산 불가 값과 이유를 구분한다.
- 외부 AI의 업체, 국가, 처리 항목, 보유기간과 자체 학습 여부 등은 실제 자료 전송 전에 검토한다.
- 카드와 리포트 생성 모델, 큐와 재시도/취소/복구, 인증, 로컬 DB와 실제 저장 연동은 미정이다.

동의와 원본 임시 처리 기준은 [ADR-001](decisions/ADR-001-consent-and-temporary-processing.md), 절차와 열린 쟁점은 [법률 문서](../legal/README.md)를 따른다.

## 상세 문서

| 필요한 내용 | 정본 |
| --- | --- |
| 역할과 지원 관계 | [PM 역할표](../pm/README.md#역할) |
| 요구사항과 검증 기준 | [테크스펙](../tech-spec.md) |
| 객체, 필드와 상태 | [공통 데이터 계약](data-contracts.md) |
| 화면용 합성 자료 | [목 데이터](mock/README.md) |
| 선택 이유와 과거 결정 | [ADR 목록](decisions/README.md) |

공통 계약은 FE, AI, BE가 공동 검토한다. 하위 호환성을 깨는 변경은 계약, 관련 README와 ADR을 함께 갱신한다.
