...
STT와 VLM은 온프레미스 GPU 1대에서 처리하고, 데이터 관리도 서버 중심으로 전환한다. 데이터별 서버 저장 범위, 단말 보관 여부와 기간은 BE가 정리하고 FE, AI와 PM이 함께 확인한다. 장비, 모델, API와 운영 방식도 미정이다. [ADR-006](decisions/ADR-006-server-side-ai-processing.md)에 결정 이유와 남은 조건을 기록한다.

현재 Flutter는 합성 목 데이터를 사용하며 FastAPI는 상태 확인과 공통 오류 및 로그의 기준 골격이다. 아래는 개발할 목표 흐름이며 실제 서버와 저장소 연결이 완료됐다는 뜻은 아니다.
...
- 카드와 리포트 생성 모델, 큐와 재시도/취소/복구, 인증, DB와 실제 저장 연동은 미정이다. GPU 1대 결정으로 DB까지 같은 장비에 배치한다고 정한 것은 아니다.
