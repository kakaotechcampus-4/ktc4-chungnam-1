...
- 구글 로그인으로 처음 들어온 계정은 그 자리에서 만들어진다. 별도의 회원가입 단계를 두지 않으며 필수 동의 확인은 최초 1회만 거친다.
...
- 인증은 구글 소셜 로그인을 사용한다. 2026-09-15 BE 리더가 정했다. 아이디와 비밀번호는 받지 않으며 계정은 첫 구글 로그인 시점에 만들어진다.
- `authProvider`의 값은 현재 `google` 하나다. 다른 제공자를 더하는 것은 계약 변경으로 본다.
- `displayName`과 `email`은 구글 계정에서 받은 값을 초기값으로 채운다. `displayName`에는 실명이 들어올 수 있으므로 사용자가 수정할 수 있어야 한다.
...
- 앱은 구글에서 받은 ID 토큰을 백엔드로 보내고, 백엔드가 구글 공개키로 검증한 뒤 계정을 찾거나 만든다. 제공자 식별자(구글 `sub`)와 ID 토큰은 이 계약에 포함하지 않는다.
