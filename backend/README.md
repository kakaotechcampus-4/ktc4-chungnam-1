...
상태: 로컬 FastAPI 기준 골격 구성, STT와 VLM의 서버 측 처리는 ADR-006으로 확정, 운영 위치와 배포 방식은 미정
...
- STT와 VLM은 서버 측에서 처리하며 서버 운영 위치와 업체는 미확정
...
서버 운영 위치와 외부 서비스 사용 여부는 PM, AI와 BE가 범위와 데이터 경계를 확인한 뒤 정한다. 필수 동의 확인, 전송 암호화, 원본과 임시 파일의 최대 24시간 삭제, 작업 큐와 로그의 삭제 확인을 완료 조건에 포함한다. 외부 업체는 업체명, 국가, 처리 목적, 항목, 보유기간과 자체 학습 여부를 확정하기 전 호출하지 않는다.
