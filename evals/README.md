...
STT 케이스(`stt-001`~`stt-006`)가 참조하는 wav 파일은 **저장소에 커밋하지 않고, 클론 후 스크립트로
...
python generate_test_cases.py        # 01_*.wav ~ 05_*.wav 생성
```

- 실행 위치: 반드시 `evals/fixtures/speech/`에서 실행 (스크립트가 현재 폴더 기준 상대 경로를 사용)
- 위 절차를 2026-09-12에 실제로 처음부터 끝까지 실행해 확인함: 새로 생성된 6개 파일이 이전에
  로컬에 있던 파일과 MD5까지 동일함 (재현 확인됨)
...
상태: STT 케이스 6개(`cases/stt-001`~`stt-006`)의 자료와 기대 결과는 준비됨. 실제 STT 결과를
