...
| 영역 | 현재 구현 |
| --- | --- |
| FE | 합성 목 데이터 화면, 생애 정보 입력과 카드 평가 구분, 홈의 리포트 상태 표시 |
| BE | FastAPI 상태 확인, 공통 오류 및 500 요청 로그, 구글 인증 API, PostgreSQL 개발 스키마와 Alembic |
| AI | STT 평가 자료, VLM 후보 비교와 이미지 분석 실험 기록 |
| 공통 | 데이터 계약, 합성 목 데이터, ADR와 협업 규칙 |

BE 인증 API는 개발용 메모리 저장소를 사용한다. 앱의 실제 구글 로그인 연결은 PR #50에서 검토 중이며, 사용자 데이터의 DB 영속 저장, 녹음과 STT 연동, AI 생성 파이프라인과 운영 배포는 아직 완성되지 않았다. 문서의 서버 중심 관리 방향은 구현 완료를 뜻하지 않는다.
